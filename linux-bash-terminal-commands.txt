...
$ cat foo.txt | more				Print a visible text on video and with pressing 'enter' the user can see more      
                                              text of this file
$ cat -n foo.txt				Print a file named foo.txt with number lines
$ cat foo.txt > bar.txt			Print an overwrited file (overwrite the content of the 2nd file with the   
                                              content of the 1st file)
$ cat > foobar.txt				The user can type the content that will be showed into this file with the     
